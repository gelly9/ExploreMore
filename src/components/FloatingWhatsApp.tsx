...
export function FloatingWhatsApp({ t }: { t: Content }) {
  ...
  // Springy ease-out when opening (slight overshoot), quicker ease-in to close.
  const ease = open ? "cubic-bezier(0.34, 1.56, 0.64, 1)" : "cubic-bezier(0.4, 0, 1, 1)";
    ...
    <div className="fixed bottom-5 right-5 z-40">
      ...
      <div className="flex flex-col items-end gap-3">
        ...
        <a
          ...
          className={`flex h-12 w-12 origin-bottom items-center justify-center rounded-full bg-lime text-charcoal shadow-lg ring-1 ring-black/10 transition-all hover:scale-105 active:scale-95 ${
            open
              ? "translate-y-0 scale-100 opacity-100"
              : "pointer-events-none translate-y-1 scale-50 opacity-0"
          }`}
          style={{ transitionDuration: "300ms", transitionTimingFunction: ease, transitionDelay: open ? "80ms" : "0ms" }}
        >
        ...
        <a
          ...
          className={`flex h-12 w-12 origin-bottom items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg ring-1 ring-black/10 transition-all hover:scale-105 active:scale-95 ${
            open
              ? "translate-y-0 scale-100 opacity-100"
              : "pointer-events-none translate-y-1 scale-50 opacity-0"
          }`}
          style={{ transitionDuration: "300ms", transitionTimingFunction: ease, transitionDelay: open ? "0ms" : "80ms" }}
        >
        ...
        <button
          ...
          className={`relative flex h-14 w-14 items-center justify-center rounded-full shadow-lg ring-1 ring-black/10 transition-all duration-300 ease-out hover:scale-105 active:scale-95 ${
            open ? "bg-charcoal text-white" : "bg-lime text-charcoal"
          }`}
        >
          {/* Close (✕) — fades/rotates in when open */}
          <span
          ...
          {/* Bike — fades/rotates out as it opens */}
          <span
            className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ease-out ${
